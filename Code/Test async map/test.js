...
Nếu return 1 promise phải có await vì nếu k có chắc chắn ra type promise chả làm được gì cả. Nếu như v, bên ngoài hàm gọi nó vẫn phải bao nó bằng 1 cái await
Thứ tự chạy luôn là tuần tự, khi dùng map sẽ là tuần tự theo index tăng dần. Khi nào gặp hàm promise or async sẽ ngay lập tức chạy trên 1 luồng riêng nếu k có await
...
=> nếu hàm số lại là promise tiếp sẽ chờ cho promise thực hiện xong là ok. Nếu hàm số là có 1 promise gọi 1 promise khác thì cái promise cấp độ 2 nó kệ mẹ, khi promise cấp độ 1 xong là thoát luôn. Do đó nếu muốn chờ promise cấp độ 2 thì luôn có await (nếu promise cấp độ 2 chỉ có 1) or await Promise.all([]) tiếp (nếu promsie cấp độ 2 có nhiều)
K quan trọng array.map(async item => {})) hay array.map(item => {<gọi hàm async>}) vì nó cứ làm như quy tắc tuần tự, gặp promise khi nào thì tách riêng luồng mới và chạy tiếp luồng cũ như bth. Thg dùng cách 1
